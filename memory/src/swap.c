...
void actualizarSwap(void* datos, uint32_t posicion_swap, uint32_t bit_modificado, uint32_t pid, uint32_t marco){
    if(bit_modificado == 1){
        usleep(cfg_memory->RETARDO_SWAP * 1000);
        pthread_mutex_lock(&mutex_archivo_swap);
        ...
        pthread_mutex_unlock(&mutex_archivo_swap);
        log_info(logger_memory,"SWAP OUT -  PID: <%d> - Marco: <%d> - Page Out: <SEGMENTO>|<PÁGINA>", pid, marco);
    }
