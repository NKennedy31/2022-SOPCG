...
bool escribir_memoria_contigua(uint32_t posicion, uint32_t valor, uint32_t marco) {
    ...
    pthread_mutex_lock(&MUTEX_MP);
    int offset = 0;
    char* stringAImprimir = string_new();
    int size =cfg_memory->TAM_PAGINA/sizeof(uint32_t);
    for (int i = 0; i <size ; i++) {
        uint32_t dato;
        memcpy(&dato,memoria_contigua + offset,sizeof(uint32_t));
        offset += sizeof(uint32_t);
        char* toString = string_itoa(dato);
        string_append(&stringAImprimir,toString);
        if(i+1<size){
            string_append(&stringAImprimir,", ");
        }

    }
    log_info(logger_memory, "Datos del marco: \n <%s>",stringAImprimir);
    pthread_mutex_unlock(&MUTEX_MP);
    /*
...
bool cargarDatosPaginaEnMarco(void* pagina, int marco){
    ...
    pthread_mutex_unlock(&MUTEX_MP);
    log_info(logger_memory,"Se cargo datos de la pagina en el marco: <%d>", marco);
    return true;
}
...
int cargarEnMP(uint32_t pid, uint32_t indiceTablaDePaginas, uint32_t indicePagina){
    ...
    for (int i = 0; i <size ; i++) {
        uint32_t dato;
        memcpy(&dato,datos + offset,sizeof(uint32_t));
        offset += sizeof(uint32_t);
        char* toString = string_itoa(dato);
        string_append(&stringAImprimir,toString);
        if(i+1<size){
    ...
    log_info(logger_memory, "Datos de la pagina: \n <%s>",stringAImprimir);
