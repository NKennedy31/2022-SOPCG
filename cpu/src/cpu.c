    ...
    while(1){
        ...
        if(cod_op>0){
            if(cod_op == INTERRUPCION){
                recibirOrden(fd_kernel_interrupt);
                log_info(logger_cpu,"Me llego una interrupcion");
                interrupcion= 1;
            }else{
                log_error(logger_cpu, "Error");
            }
...
void *atenderPcb(){
    while(1){
        uint32_t cod_op= recibir_operacion(fd_kernel_dispatch);
        if(cod_op>0)
        {
            switch (cod_op)
            {
                case PCB:{
                    pcb_actual = recibir_pcb(fd_kernel_dispatch);
                    log_info(logger_cpu,"Recibi un proceso:");
                    log_info(logger_cpu,"id: %d",pcb_actual->id);
                    ciclo_instrucciones();
                    break;
                }
...
void ciclo_instrucciones(){
    while(1)
    {
        ...
        if(interrupcion == 1){
            //ACA DEVOLVEMOS LA PCB. SEGUN EL PC QUE OPCODE LE PONEMOS
            //
            devolverPcb(PROCESO_DESALOJADO,fd_kernel);
            interrupcion = 0;
            return;
        }else if(devolviLaPcb){
            devolviLaPcb = false;
            return;
        }
...
void escribir_en_registro(char* registro, uint32_t valor){
    if(strcmp(registro, "AX") == 0){
        pcb_actual->registrosCpu->AX = valor;
    }else if(strcmp(registro, "BX") == 0){
        pcb_actual->registrosCpu->BX = valor;
    }else if(strcmp(registro, "CX") == 0){
        pcb_actual->registrosCpu->CX = valor;
    }else if(strcmp(registro, "DX") == 0){
        pcb_actual->registrosCpu->DX = valor;
    }
...
uint32_t leer_de_registro(char* registro){
    ...
    if(strcmp(registro, "AX") == 0){
        valor_registro = pcb_actual->registrosCpu->AX;
    }else if(strcmp(registro, "BX") == 0){
        valor_registro = pcb_actual->registrosCpu->BX;
    }else if(strcmp(registro, "CX") == 0){
        valor_registro = pcb_actual->registrosCpu->CX;
    }else if(strcmp(registro, "DX") == 0){
        valor_registro = pcb_actual->registrosCpu->DX;
    }
...
void buscar_marco(){
    ...
    if (respuesta_tlb < 0) { //TLB MISS o NO HAY TLB
        ...
        op_code cop;
        recv(fd_memoria, &cop, sizeof(op_code), 0);
        ...
        switch (cop) {
            ...
            case ACCESO_RESULTO_PAGE_FAULT:{
                uint32_t valor = recibirValor_uint32(fd_memoria, logger_cpu);
...
void page_fault(){
    ...
    enviar_paquete_pcbPf(pcbPF,fd_kernel,BLOCKED_PF,logger_cpu);
    interrupcion = 0;
    devolviLaPcb = true;
    liberarPcb(pcb_actual);
    free(pcbPF);
...
void ejecutar_escritura() {
    ...
    log_info(logger_cpu, "Escritura de Memoria: envio el valor: <%d> en la Direccion Fisica: <%d> ", valor,
             dir_fisica_actual);
